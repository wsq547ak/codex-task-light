...
当前选择会写入 `./.runtime/preferences.json`，下次启动时自动恢复。
...
- 点击当前状态灯
- 在 `显示方式` 下选择 `状态栏` 或 `悬浮窗`
- 勾选或取消 `窗口置顶`
- 选择后应立即切换到对应显示方式或置顶状态
...
- `状态栏`：显示在 macOS 菜单栏
- `悬浮窗`：显示普通可拖动悬浮窗，可能被其他窗口遮住
- `窗口置顶`：仅对悬浮窗生效；开启后尽量保持在其他窗口上方，默认开启
